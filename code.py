...
state2StartTime = 0 #stores time when program enters the 10 second waiting time
lastObjectDescription = "" #stores result of 1st AI scan
...
# ---------------------------------------------------------
# ADDED HELPER: FIRST AI SCAN
# This is the object description state from the diagram
# ---------------------------------------------------------
def firstScan(): # captures image, converts image, image to AI, AI describes image/object , stores in lastObjectDescription, output result
...
# SECOND AI SCAN(follow up of location of object at current moment)
...
#  button callback,follows states: first ftate,first scan, second state, 10 sec timer

def detect(chn):
    global state
    global state2StartTime

    try:
        # State 0 --> State 1
        # Not in scanning state --> enter camera scan state
        if state == 0:
        ...
        # Waiting for second input -> follow-up AI state
            ...
            # After second scan, return to normal state
            state = 0
...
def loop():
    ...
    while True:
        ...
        # --------------------------------------
        # ADDED: 10-second timeout for state 2
        # --------------------------------------
        if state == 2:
